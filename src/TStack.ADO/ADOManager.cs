...
{
    public class ADOManager<TContext> : ADOManager
    ...
    public class ADOManager : ADOBaseManager
    {
        ...
        public T ExecuteScalar<T>(string command, TStack.ADO.Tool.CommandType commandType, List<Parameter> parameters)
        {
            var sqlCommand = CommandBuilder(command, commandType, parameters);
            return ExecuteScalar<T>(sqlCommand);
        }
        public void Execute(string command, TStack.ADO.Tool.CommandType commandType, List<Parameter> parameters)
        {
            var sqlCommand = CommandBuilder(command, commandType, parameters);
            Execute(sqlCommand);
        }
        public T Execute<T>(string command, TStack.ADO.Tool.CommandType commandType, List<Parameter> parameters)
        {
            var sqlCommand = CommandBuilder(command, commandType, parameters);
            return Execute<T>(sqlCommand);
        }
        public DataTable GetDataTable(string command, TStack.ADO.Tool.CommandType commandType, List<Parameter> parameters)
        {
            var sqlCommand = CommandBuilder(command, commandType, parameters);
            return GetDataTable(sqlCommand);
        }
        public DataSet GetDataSet(string command, TStack.ADO.Tool.CommandType commandType, List<Parameter> parameters)
        {
            var sqlCommand = CommandBuilder(command, commandType, parameters);
